// ─────────────────────────────────────────────────────────────────
// Marketing Poster Generator + Zernio Batch Scheduler — single-file version
// Search query in -> Pexels stock photo -> branded image -> upload to
// Zernio -> scheduled post. Generic promo poster: headline + copy + CTA.
//
// Setup:
...
async function searchPexelsPhoto(query, apiKey, { orientation = 'portrait' } = {}) {
  if (!apiKey) throw new Error('Missing PEXELS_API_KEY');
  if (!query) throw new Error('Missing search query');

  const url = `${PEXELS_BASE}/search?query=${encodeURIComponent(query)}&per_page=10&orientation=${orientation}`;
  ...
  return {
    ...
    sourcePageUrl: photo.url // Pexels requires attribution — keep this around
  };
}
...
async function generateAndSchedule({
  ...
  const photo = await searchPexelsPhoto(query, PEXELS_API_KEY, { orientation });
  ...
  const safeSlug = (hook || query).replace(/\s+/g, '_').slice(0, 40);
  ...
  if (mediaKind === 'video') {
    ...
    publicUrl = await uploadToZernio(buffer, `${safeSlug}_${Date.now()}.mp4`, contentType);
  } else {
    buffer = imageBuffer;
    contentType = 'image/jpeg';
    zernioMediaType = 'image';
    publicUrl = await uploadToZernio(buffer, `${safeSlug}_${Date.now()}.jpg`, contentType);
  }
...
// 20-item batch spreads its Pexels/ffmpeg/Zernio work across the whole
    ...
    <html>
      ...
      <body>
        ...
        <form action="/generate" method="GET" target="_blank">
          <label>Pexels search query</label>
          <input name="query" placeholder="coffee shop morning" required/>
        ...
        <p class="hint">Caption posted to Zernio = Copy + CTA only, no hashtags. The Hook is baked into the image/video as on-screen text and is never repeated in the caption.</p>
...
skincare routine | Glow starts here | Clinically tested, dermatologist approved. | Try It Free"></textarea>
        ...
        <div id="jobsTableWrap">
          <table id="jobsTable">
            <thead>
              <tr>
                <th>Status</th><th>Hook</th><th>Media</th><th>Platform</th>
        ...
        <script>
          ...
          btn.addEventListener('click', async () => {
            ...
            if (!items.length) {
              progressEl.textContent = 'No valid lines found. Use: Query | Hook | Copy | CTA';
              return;
            }
...
app.get('/generate', async (req, res) => {
  try {
    ...
    res.set('Content-Disposition', `inline; filename="${hook.replace(/\s+/g, '_').slice(0, 40)}.jpg"`);
...
app.get('/generate-video', async (req, res) => {
  try {
    ...
    res.set('Content-Disposition', `inline; filename="${hook.replace(/\s+/g, '_').slice(0, 40)}.mp4"`);
...
// Batch schedule: takes a list of { query, hook, copy, cta } items and
// enqueues one job per item. Each job only generates its poster/video,
// uploads it to Zernio, and creates the scheduled post once `generateAt`
// arrives — by default `leadMinutes` before its `scheduledFor` slot —
// instead of doing all the heavy work upfront. Returns immediately with
// a batchId to poll.
//
// Body:
// {
...
app.post('/schedule-batch', async (req, res) => {
  try {
    ...
    if (!Array.isArray(items) || !items.length) {
      return res.status(400).json({ error: 'items must be a non-empty array of { query, hook }' });
    }
    ...
    const newJobs = items.map(item => {
      ...
      const hasRequiredFields = Boolean(item.query && item.hook);
      ...
      return {
        ...
        error: hasRequiredFields ? null : 'Each item needs a query and hook',
        result: null
      };
    });
    ...
    res.json({
      ...
      jobs: newJobs.map(j => ({
        id: j.id, hook: j.hook, mediaKind: j.mediaKind,
...
app.get('/jobs', (req, res) => {
  ...
  res.json({
    ...
    jobs: jobs.map(j => ({
      ...
      hook: j.hook, mediaKind: j.mediaKind, platform: j.platform,
      scheduledFor: j.scheduledFor, generateAt: j.generateAt,
...
loadQueue()
  ...
  .then(() => {
    ...
    app.listen(PORT, () => {
      console.log(`Marketing poster generator running at http://localhost:${PORT}`);
    });
  })
  .catch(err => {
